...
cfg.OutputFile = fullfile(cfg.OutputDir, "datassb_fixedfreq_dynamictiming_fast_" + string(timestamp) + ".mat");
...
function result = processOneWaveformFixedSync(waveform, cfg, syncState)
    ...
    try
        ...
        %% Align waveform with dynamic timing
        t0 = tic;

        if timingOffset < 0 || timingOffset >= size(correctedWaveform, 1)
            error("Invalid timingOffset: %d", timingOffset);
        end
