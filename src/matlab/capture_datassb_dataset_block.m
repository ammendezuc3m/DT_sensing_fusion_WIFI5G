%% Dataset block capture for fast dataSSB sensing
% This script captures one dataset block for one label.
%
% It is designed to be launched from terminal/mobile using environment variables:
%
%   CAPTURE_LABEL=empty CAPTURE_BLOCK=1 matlab -batch "run('src/matlab/capture_datassb_dataset_block.m')"
%
% Main output:
%   dataSSB: 360 x 6 x N complex single
%
% Useful SSB region after validation:
%   full 30-RB grid: dataSSB(:, :, i)
%   pure SSB:        dataSSB(61:300, 2:5, i)
%
% Pipeline:
%   1) Warmup: estimate fixed frequency offset and NID2.
%   2) Main loop:
%        - capture 20 ms
%        - apply fixed frequency correction
%        - estimate timing dynamically per capture
%        - align waveform
%        - OFDM demodulate 30 RB
%        - save first 6 OFDM symbols as dataSSB
%   3) Post-capture validation:
%        - PSS/SSS correlation
%        - optional PBCH/BCH CRC check
%
% No countdown. Capture starts immediately after SDR setup and warmup.
...
cfg.OutputFile = fullfile(cfg.BlockDir, cfg.SessionId + ".mat");
cfg.MetadataFile = fullfile(cfg.BlockDir, cfg.SessionId + "_metadata.json");
...
for captureIdx = 1:N
    ...
    tLoop = tic;
    captureUnixTime(captureIdx) = posixtime(datetime("now"));
